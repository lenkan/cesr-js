...
def format_size(
    sizes: dict[str, Cizage | Xizage | Sizage], hards: dict[str, int]
) -> str:
    result = dict()
    result["hards"] = hards
    result["sizes"] = dict()
...
print(f"export const MatterTable = {format_size(Matter.Sizes, Matter.Hards)}")
print(
    f"export const CountTable_10 = {format_size(Counter.Sizes.get(1).get(0), Counter.Hards)}"
)
print(
    f"export const CountTable_20 = {format_size(Counter.Sizes.get(2).get(0), Counter.Hards)}"
)
print(f"export const IndexTable = {format_size(Indexer.Sizes, Indexer.Hards)}")
