...
export class Encoder {
  encode(frame: FrameData, table: CodeTable): string {
  ...
  encodeIndexedSignature(alg: MatterSignature, raw: Uint8Array, index: number): string {
    switch (alg) {
      case "ed25519":
        return this.encodeIndexer({ code: IndexCode.Ed25519_Sig, raw, index });
      default:
        throw new Error(`Unsupported signature algorithm: ${alg}`);
    }
  }

  encodeMatter(raw: MatterInit): string {
    return this.encode(raw, MatterTable);
  }
  ...
  encodeDate(date: Date): string {
    ...
    return this.encodeMatter({ code: MatterCode.DateTime, raw });
  }
  ...
  encodeString(txt: string): string {
    ...
    switch (leadSize) {
      case 0:
        return this.encodeMatter({ code: MatterCode.StrB64_L0, raw });
      case 1:
        return this.encodeMatter({ code: MatterCode.StrB64_L1, raw });
      case 2:
        return this.encodeMatter({ code: MatterCode.StrB64_L2, raw });
      default:
        throw new Error(`Could not determine lead size`);
    }
  }

  encodeSignature(alg: MatterSignature, data: Uint8Array): string {
    switch (alg) {
      case "ed25519":
        return this.encodeMatter({ code: MatterCode.Ed25519_Sig, raw: data });
      case "secp256k1":
        return this.encodeMatter({ code: MatterCode.ECDSA_256k1_Sig, raw: data });
      default:
        throw new Error(`Unsupported signature algorithm: ${alg}`);
    }
  }
  encodeDigest(alg: MatterDigest, data: Uint8Array): string {
    switch (alg) {
      case "blake3_256":
        return this.encodeMatter({ code: MatterCode.Blake3_256, raw: data });
      case "blake3_512":
        return this.encodeMatter({ code: MatterCode.Blake3_512, raw: data });
      default:
        throw new Error(`Unsupported digest algorithm: ${alg}`);
    }
  }

  encodeGenus(genus: GenusInit): string {
  ...
  encodeCounterV1(raw: CounterInit): string {
    return this.encode(raw, CountTable_10);
  }

  encodeCounterV2(raw: CounterInit): string {
    return this.encode(raw, CountTable_20);
  }

  encodeAttachmentsV1(count: number) {
    if (count > 64 ** 2) {
      return this.encodeCounterV1({ code: CountCode_10.BigAttachmentGroup, count });
    }

    return this.encodeCounterV1({ code: CountCode_10.AttachmentGroup, count });
  }

  encodeAttachmentsV2(count: number) {
    if (count > 64 ** 2) {
      return this.encodeCounterV2({ code: CountCode_20.BigAttachmentGroup, count });
    }

    return this.encodeCounterV2({ code: CountCode_20.AttachmentGroup, count });
  }
...
export class Decoder {
  ...
  decode(input: Uint8Array | string, table: CodeTable): Required<FrameData> {
    const result = this.decodeStream(input, table);
  ...
  decodeStream(input: Uint8Array | string, table: CodeTable): DecodeStreamResult {
    ...
    const hs = this.#findHardSize(input, table);
  ...
  decodeIndexer(input: Uint8Array | string): Indexer {
    return this.decode(input, IndexTable);
  }

  decodeGenus(input: string): Genus {
    const major = decodeBase64Int(input.slice(5, 6));
    const minor = decodeBase64Int(input.slice(6, 8));
    return { major, minor };
  }

  decodeMatter(input: Uint8Array | string): Matter {
    return this.decode(input, MatterTable);
  }

  decodeCounterV1(qb64: string | Uint8Array): Counter {
    return this.decode(qb64, CountTable_10);
  }
