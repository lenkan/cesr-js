...
describe(basename(import.meta.url), () => {
  describe("encoding matter primitives", () => {
    test("should throw if raw does not have enough bytes", () => {
      ...
      const matter = Matter.create.ed25519_sig(new Uint8Array(63));
    ...
    test("should encode fixed size primitive", () => {
      const matter = Matter.create.blake3_256(new Uint8Array(32));
    ...
    test("should encode 2 char code fixed size primitive", () => {
      const matter = Matter.create.ed25519_sig(new Uint8Array(64));
    ...
    test("should encode small string lead 0", () => {
      const matter = Matter.create.string("fo!");
    ...
    test("should encode small string lead 1", () => {
      const matter = Matter.create.string("fooo!");
    ...
    test("should encode small string lead 2", () => {
      const matter = Matter.create.string("fooooo!");
  ...
  describe("encode tags", () => {
    test("should encode single character tag", () => {
      const matter = Matter.create.tag("a");
  ...
  describe("encoding numbers", () => {
    test("should encode 0", () => {
      assert.strictEqual(Matter.create.decimal(0).text(), "6HABAAA0");
    });
    ...
    test("should encode 0.1", () => {
      assert.strictEqual(Matter.create.decimal(0.1).text(), "4HABA0p1");
    });

    test("should encode 1", () => {
      assert.strictEqual(Matter.create.decimal(1).text(), "6HABAAA1");
    });

    test("should encode 123", () => {
      assert.strictEqual(Matter.create.decimal(123).text(), "4HABA123");
    });

    test("should encode 1.1", () => {
      assert.strictEqual(Matter.create.decimal(1.1).text(), "4HABA1p1");
    });

    test("should encode -1.1", () => {
      assert.strictEqual(Matter.create.decimal(-1.1).text(), "4HAB-1p1");
    });

    test("should encode 12345678", () => {
      assert.strictEqual(Matter.create.decimal(12345678).text(), "4HAC12345678");
    });

    test("should encode MAX_SAFE_INTEGER", () => {
      assert.strictEqual(Matter.create.decimal(Number.MAX_SAFE_INTEGER).text(), "4HAE9007199254740991");
    });
  });

  describe("encode/decode hex numbers", () => {
    test("should encode/decode 0x00", () => {
      const matter = Matter.create.hex("0");
    ...
    test("should encode/decode single digit number", () => {
      const matter = Matter.create.hex("2");
    ...
    test("should encode/decode hex number", () => {
      const matter = Matter.create.hex("32");
    ...
    test("should encode/decode large hex number", () => {
      ...
      const matter = Matter.create.hex(hex);
  ...
  describe("encoding strings", () => {
    test("should encode/decode 'abc'", () => {
      ...
      const matter = Matter.create.string(value);
  ...
  describe("encoding dates", () => {
    test("should encode CESR date format", () => {
      const matter = Matter.create.date(new Date(Date.parse("2024-11-23T16:02:27.123Z")));
    ...
    test("should decode an encoded date", () => {
      ...
      const matter = Matter.create.date(original);
    ...
    test("should handle dates with zero milliseconds", () => {
      ...
      const matter = Matter.create.date(date);
      const decoded = matter.decode.date();
    ...
    test("should throw error for invalid date", () => {
      ...
      assert.throws(() => {
        Matter.create.date(invalidDate);
      }, /Invalid date/);
    });
    ...
    test("should round-trip multiple times consistently", () => {
      ...
      for (let i = 0; i < 3; i++) {
        const encoded = Matter.create.date(current);
    ...
    test("should handle leap year date", () => {
      ...
      const matter = Matter.create.date(leapDay);
